...
class FISTALassoBoostInc(FISTABase):
    def __init__(self, _run, n_labeled, boost_type=' xgb'):
        self._run = _run
        # Load configuration.
    ...
    def __str__(self):
        return 'FISTA Lasso Boost Inc of type %s with lambda_1[%0.2f], lambda_2[%0.2f], n_labeled[%d]' % (self.boost_type, self.lambda_1,
                                                                                               self.lambda_2,
                                                                                               self.n_labeled)
    ...
    def gradient_boosting(self, X_l, X_u, L, y_l, y_u, f_p_l, f_p_u, x_del_f, m, return_yl=False):
        ...
        self._run.log_scalar(
            '%s.fista_lasso_xgb_%s.mse.%0.2f.%0.2f' % (
            self.boost_type, self.data_set_name, self.lambda_1, self.lambda_2),
            mean_squared_error(b_y_u, y_pred), step=m)

        if return_yl:
            y1 = clf.predict(X_l)
            return np.hstack((y1, y_pred))
        else:
            return y_pred
    ...
    def exec(self, X_l, X_u, L, y_l, y_u, y_p_l, y_p_u, f_p_l, f_p_u, random_state):
        ...
        self._run.log_scalar(
            '%s.fista_lasso_boost_%s.s_acc.%0.2f.%0.2f.%0.2d' % (
            self.boost_type, self.data_set_name, self.lambda_1, self.lambda_2, random_state),
            s_acc, step=m)
        self._run.log_scalar(
            '%s.fista_lasso_boost_%s.s_acc_cen.%0.2f.%0.2f.%0.2d' % (
            self.boost_type, self.data_set_name, self.lambda_1, self.lambda_2, random_state),
            s_acc_cen, step=m)
        ...
        for i in range(5):
            f_val, l_val, x_del_f = self.execute_algorithm(X_l, X_u, L, y, f_p, self.lambda_1, self.lambda_2,
                                                           self.n_iterations)
            p_y = self.gradient_boosting(X_l, X_u, L, y_l, y_u, f_p_l, f_p_u, x_del_f, m, return_yl=True)
            p_y = np.reshape(p_y, (p_y.shape[0], 1))
            f_p = np.add(f_p, p_y)
            ...
            print(f_acc, f_acc_cen)
            print('----------')

        self._run.log_scalar(
            '%s.fista_lasso_boost_%s.f_acc.%0.2d.%0.2f.%0.2f' % (
            self.boost_type, self.data_set_name, random_state, self.lambda_1, self.lambda_2),
            f_acc, step=m)
        self._run.log_scalar(
            '%s.fista_lasso_boost_%s.f_acc_cen.%0.2d.%0.2f.%0.2f' % (
            self.boost_type, self.data_set_name, random_state, self.lambda_1, self.lambda_2),
            f_acc_cen, step=m)
